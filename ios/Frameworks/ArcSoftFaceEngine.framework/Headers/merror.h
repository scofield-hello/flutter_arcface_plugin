...
#define MERR_FSDK_MISMATCH_ID_AND_SDK			(MERR_FSDK_BASE + 4)		//SDKKey 和使用的SDK 不匹配（注意：调用初始化引擎接口时，请确认激活接口传入的参数，并重新激活）
...
#define MERR_ASF_UNIQUE_IDENTIFIER_MISMATCH				(MERR_ASF_BASE+7)				//唯一标识不匹配（弃用）
...
#define MERR_ASF_REQUEST_TIMEOUT						(MERR_ASF_BASE_EXTEND+6)		//客户端时间与服务器时间（即北京时间）前后相差在30分钟之内
#define MERR_ASF_APPID_DATA_DECRYPT						(MERR_ASF_BASE_EXTEND+7)		//服务端解密失败
#define MERR_ASF_APPID_APPKEY_SDK_MISMATCH				(MERR_ASF_BASE_EXTEND+8)		//传入的AppId和AppKey与使用的SDK版本不一致
#define MERR_ASF_NO_REQUEST								(MERR_ASF_BASE_EXTEND+9)		//短时间大量请求会被禁止请求,30分钟之后会解封
...
#define MERR_ASF_ACTIVEKEY_COULDNT_CONNECT_SERVER		(MERR_ASF_ACTIVEKEY_BASE+1)		//无法连接激活码服务器
...
#define MERR_ASF_ACTIVEKEY_PARM_MISMATCH				(MERR_ASF_ACTIVEKEY_BASE+4)		//激活码正确，且未被使用，但和传入的APPID及APPKEY不匹配
#define MERR_ASF_ACTIVEKEY_ACTIVEKEY_ACTIVATED			(MERR_ASF_ACTIVEKEY_BASE+5)		//传入的KEY值虽然正确，但此KEY已经被激活
#define MERR_ASF_ACTIVEKEY_ACTIVEKEY_FORMAT_ERROR		(MERR_ASF_ACTIVEKEY_BASE+6)		//KEY格式不对，一般来说是KEY错误或者未传入KEY值
#define MERR_ASF_ACTIVEKEY_APPID_PARM_MISMATCH			(MERR_ASF_ACTIVEKEY_BASE+7)		//激活码正确，被使用，但传入的APPID不匹配
#define MERR_ASF_ACTIVEKEY_PAYSDK_FREEFILE_MISMATCH		(MERR_ASF_ACTIVEKEY_BASE+8)     //付费版sdk不能使用免费版本的激活文件
#define MERR_ASF_ACTIVEKEY_EXPIRED						(MERR_ASF_ACTIVEKEY_BASE+9)		//激活码已过期，请更换新的激活码


#define MERR_ASF_OFFLINE_BASE							0x19000							//离线激活错误码类型
#define MERR_ASF_LICENSE_FILE_NOT_EXIST					(MERR_ASF_OFFLINE_BASE+1)		//web端下载的许可文件不存在
#define MERR_ASF_LICENSE_FILE_DATA_DESTROYED			(MERR_ASF_OFFLINE_BASE+2)		//web端下载的许可文件已损坏
#define MERR_ASF_LICENSE_FILE_SDK_MISMATCH				(MERR_ASF_OFFLINE_BASE+3)		//web端下载的许可文件与SDK版本不匹配
#define MERR_ASF_LICENSE_FILEINFO_SDKINFO_MISMATCH		(MERR_ASF_OFFLINE_BASE+4)		//web端下载的许可文件与SDK基础信息不匹配
#define MERR_ASF_LICENSE_FILE_FINGERPRINT_MISMATCH		(MERR_ASF_OFFLINE_BASE+5)		//web端下载的许可文件与设备指纹不匹配
#define MERR_ASF_LICENSE_FILE_PAY_EXPIRED				(MERR_ASF_OFFLINE_BASE+6)		//web端下载的付费版许可文件已过期，请重新激活
#define MERR_ASF_LOCAL_EXIST_USEFUL_ACTIVE_FILE			(MERR_ASF_OFFLINE_BASE+7)		//web端下载的许可文件不可用，本地存在激活文件可用（等价已激活）
